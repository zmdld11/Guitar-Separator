...
生成时间: 2025-12-11 15:06:34
不知道为什么模型分不出funk吉他、钢琴、三角铁
...
class GuitarDataPreprocessor:
    ...
    def load_data_from_json(self, max_samples_per_instrument=None, balance_classes=False, 
                           guitar_multiplier=1, max_total_samples=5000):
        ...
        for item in data:
            ...
            if label == 1:  # 吉他
                positive_samples.append((file_path, label))
            else:  # 非吉他
                negative_samples.append((file_path, label))
                
                # 按乐器类型统计
                instrument = item.get('instrument', 'unknown')
                if instrument not in negative_by_instrument:
                    negative_by_instrument[instrument] = []
                negative_by_instrument[instrument].append((file_path, label))
    ...
    def load_non_guitar_samples(self, max_samples=None):
        ...
        # 首先尝试从JSON加载
        json_data = self.load_data_from_json(max_samples_per_instrument=500, balance_classes=False)
        if json_data and json_data[0]:
            file_paths, labels = json_data
            # 只取负样本
            for file_path, label in zip(file_paths, labels):
                if label == 0:
                    negative_samples.append((file_path, label))
            print(f"✅ 从JSON加载 {len(negative_samples)} 个非吉他样本")
            return negative_samples
        
        # 如果JSON加载失败，回退到目录扫描
        print("⚠️  JSON加载失败，使用目录扫描方式")
    ...
    def prepare_dataset(self, max_negative_samples=None, use_json=True):
        ...
        if use_json:
            ...
            file_paths, labels = self.load_data_from_json(
                ...
                max_total_samples=4000  # 最多4000个样本
            )
...
def test_guitar_timeline_with_model(analyzer, audio_path, threshold=0.5, 
    ...
    try:
        ...
        test_positions = [0, 30, 60, 90, 120]  # 测试多个时间点
        for pos in test_positions:
            if pos < duration:
                start_sample = int(pos * sr)
                end_sample = int(min(pos + 3, duration) * sr)
                ...
                try:
                    # 测试单个片段
                    prob = analyzer.detect_guitar_probability(tmp_path)
                    print(f"📊 {pos}-{pos+3}s 吉他概率: {prob:.4f} {'🎸' if prob > threshold else '❌'}")
                finally:
                    # 确保删除临时文件
                    try:
                        os.unlink(tmp_path)
...
def main():
    ...
    if args.debug:
        print("\n🔍 调试模式：验证模型...")
        model_path = args.model_path or os.path.join(GuitarRecConfig.MODEL_DIR, "best_model.pth")
        ...
        # 加载模型并测试一个吉他样本
        from src.rec.model import SimplifiedGuitarClassifier
        model = SimplifiedGuitarClassifier().to(device)
        ...
        # 测试一个训练集中的吉他样本
        json_path = os.path.join(GuitarRecConfig.DATA_DIR, "extract", "binary_classification", "dataset.json")
        if os.path.exists(json_path):
            import json
            with open(json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            guitar_samples = [item for item in data if item['label'] == 1]
            if guitar_samples:
                sample = guitar_samples[0]
                if os.path.exists(sample['file']):
                    print(f"\n🎸 测试训练集吉他样本: {os.path.basename(sample['file'])}")
                    
                    # 提取特征
                    import librosa
                    y, sr = librosa.load(sample['file'], sr=GuitarRecConfig.TARGET_SAMPLE_RATE)
                    y = librosa.util.fix_length(y, size=GuitarRecConfig.TARGET_SAMPLE_RATE * GuitarRecConfig.AUDIO_DURATION)
                    ...
                    # 预测
                    input_tensor = torch.FloatTensor(log_mel).unsqueeze(0).unsqueeze(0).to(device)
                    with torch.no_grad():
                        outputs = model(input_tensor)
                        probabilities = torch.softmax(outputs, dim=1)
                        prob_guitar = probabilities[0, 1].item()
                    
                    print(f"📊 吉他概率: {prob_guitar:.4f}")
                    print(f"🔍 应该为吉他，模型判断: {'✅ 正确' if prob_guitar > 0.5 else '❌ 错误'}")
...
## test_model_debug.py
文件路径: test\test_model_debug.py

```py
# test/test_model_debug.py
#!/usr/bin/env python
"""
模型调试测试脚本
用于验证模型是否能正确识别吉他
...
def extract_features(audio_path, target_sr=22050, duration=3.0):
    """提取音频特征（与训练时一致）"""
    try:
        # 加载音频
        ...
        # 确保音频长度一致
        y = librosa.util.fix_length(y, size=int(target_sr * duration))
...
def test_single_audio(model, device, audio_path, threshold=0.5):
    """测试单个音频文件"""
    ...
    print(f"🔍 结果: {'🎸 是吉他' if prob_guitar > threshold else '❌ 不是吉他'}")
    
    return prob_guitar

def test_random_samples(model, device, num_samples=10):
    """随机测试训练集中的样本"""
    print("\n" + "="*60)
    print("🎸 随机测试训练集样本")
    print("="*60)
    ...
    # 分离正负样本
    guitar_samples = [item for item in data if item['label'] == 1]
    non_guitar_samples = [item for item in data if item['label'] == 0]
    
    print(f"📊 数据集统计:")
    print(f"  吉他样本数: {len(guitar_samples)}")
    print(f"  非吉他样本数: {len(non_guitar_samples)}")
    
    # 随机选择样本测试
    test_samples = []
    if guitar_samples:
        test_samples.extend(random.sample(guitar_samples, min(num_samples//2, len(guitar_samples))))
    if non_guitar_samples:
        test_samples.extend(random.sample(non_guitar_samples, min(num_samples//2, len(non_guitar_samples))))
    
    results = []
    for i, item in enumerate(test_samples):
        print(f"\n[{i+1}/{len(test_samples)}] {'🎸' if item['label']==1 else '🎹'} {os.path.basename(item['file'])}")
        
        if not os.path.exists(item['file']):
            print(f"  ⚠️  文件不存在")
            continue
        
        prob_guitar = test_single_audio(model, device, item['file'], threshold=0.3)
        if prob_guitar is not None:
            predicted = 1 if prob_guitar > 0.3 else 0
            correct = predicted == item['label']
            results.append((item['label'], predicted, prob_guitar, correct))
            
            status = "✅ 正确" if correct else "❌ 错误"
            print(f"  {status} | 真实: {'吉他' if item['label']==1 else '非吉他'}, 预测: {'吉他' if predicted==1 else '非吉他'}")
    
    # 计算准确率
    if results:
        correct_count = sum(1 for r in results if r[3])
        accuracy = correct_count / len(results)
        print(f"\n📈 随机测试准确率: {accuracy:.2%} ({correct_count}/{len(results)})")
...
def main():
    ...
    if not os.path.exists(model_path):
        print(f"❌ 模型文件不存在: {model_path}")
        print("请先训练模型: python src/rec/train.py")
        return
    ...
    # 1. 随机测试训练集样本
    test_random_samples(model, device, num_samples=20)
    
    # 2. 测试用户指定的音频
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--audio-path', type=str, help='测试音频路径')
    ...
    if args.audio_path and os.path.exists(args.audio_path):
        ...
        print("🎸 用户音频测试")
        print("="*60)
        test_single_audio(model, device, args.audio_path, threshold=args.threshold)
    elif not args.audio_path:
        # 如果没有指定音频，测试一些示例
        print("\n" + "="*60)
        print("💡 提示: 使用 --audio-path 参数测试自定义音频")
        print("示例: python test/test_model_debug.py --audio-path your_audio.wav --threshold 0.3")
        print("="*60)
