...
class HTDemucs(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        # ---------- 注入层（将时域特征注入频域编码器） ----------
        # 为每个频域编码层创建一个注入投影（将时域通道投影到频域通道）
        self.inject_projs = nn.ModuleList()
        time_chs = [config.time_channels * (2**i) for i in range(config.time_depth)]
        freq_chs = [config.freq_channels * (2**i) for i in range(config.freq_depth)]
        for t_ch, f_ch in zip(time_chs, freq_chs):
    ...
    def forward(self, waveform):
        ...
        spec = stft(waveform,
                    ...
                    normalized=self.config.stft_normalized)  # (b, 2, t, f)
        f = spec
        freq_skips = []
        # 频率位置嵌入
        ...
        mask = self.freq_out(f_dec)   # (b, 2, t, f)

        # 应用掩码：估计谱 = 输入谱 * mask
        # 注意：mask 可能是线性幅度或复数乘？这里直接逐元素乘（实部乘实部，虚部乘虚部）
        est_spec = spec * mask  # 广播，spec 形状 (b,2,t,f)
        # iSTFT 还原波形
        est_wave = istft(est_spec,
                         ...
                         length=waveform.size(2))  # 指定输出长度

        # 融合两个分支
        min_len = min(time_out.size(2), est_wave.size(2))
